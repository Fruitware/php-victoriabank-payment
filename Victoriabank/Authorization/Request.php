...
class Authorization_Request extends Request{
    ...
    {
        if (!isset($this->_requestFields[self::AMOUNT]) || strlen($this->_requestFields[self::AMOUNT]) < 1 || strlen($this->_requestFields[self::AMOUNT]) > 12)
        ...
        if (!isset($this->_requestFields[self::ORDER]) || strlen($this->_requestFields[self::ORDER]) < 6 || strlen($this->_requestFields[self::ORDER]) > 32)
        ...
        if (!isset($this->_requestFields[self::MERCH_URL]) || strlen($this->_requestFields[self::MERCH_URL]) < 1 || strlen($this->_requestFields[self::MERCH_URL]) > 250)
        ...
        if (!isset($this->_requestFields[self::NONCE]) || strlen($this->_requestFields[self::NONCE]) < 20 || strlen($this->_requestFields[self::NONCE]) > 32)
            throw new Exception('Authorization request failed: invalid ' . self::NONCE);

        if (!isset($this->_requestFields[self::BACKREF]) || strlen($this->_requestFields[self::BACKREF]) < 1 || strlen($this->_requestFields[self::BACKREF]) > 250)
    ...
    public function request()
    {
        ...
        $formHtml   = $form->setFormMethod('POST')->setFormAction($this->_gatewayUrl)->renderForm(!$this->_debugMode);
